import { Home, MessageSquare, FileText, Menu, RefreshCw, MapPin, Moon, Sun, ChevronLeft } from 'lucide-react';
...
export function Sidebar({ isCollapsed = false, onToggleCollapse, isMobile = false }: SidebarProps) {
  const { theme, toggleTheme } = useTheme();
  const [openGroups, setOpenGroups] = useState<string[]>(['main', 'network']);
  ...
  return (
    <div className={`${isMobile ? 'w-full' : isCollapsed ? 'w-[74px]' : 'w-full'} h-full bg-slate-800 dark:bg-slate-900 text-white transition-all duration-300 flex flex-col`}>
      ...
      <div className="p-2 border-t border-slate-700 dark:border-slate-800">
        ...
        >
          {theme === 'light' ? (
            <Moon className={`h-5 w-5 ${isCollapsed && !isMobile ? '' : 'mr-3'}`} />
          ) : (
            <Sun className={`h-5 w-5 ${isCollapsed && !isMobile ? '' : 'mr-3'}`} />
          )}
          {(!isCollapsed || isMobile) && <span>{theme === 'light' ? 'Dark Mode' : 'Light Mode'}</span>}
        </Button>
      </div>
    </div>
  );
